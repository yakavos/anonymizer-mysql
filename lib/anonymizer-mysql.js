    ...
    return new Promise(function(resolve, reject) {
      ...
      mysql.createConnection(thisSelf.config.target_db).then(function(connection) {
        var queries = thisSelf.config.rules.map(function(query_data) {
          ...
          var query = action + thisSelf.config.target_db.database + "." + query_data.table + " SET ";
          query_data.fields.forEach(function(field) {
            query += thisSelf.config.target_db.database + "." + field.field + " = " + mysql.escape(field.apply) + ", ";
          });
          var whereQuery =
          ...
          return connection.query("SET sql_notes = 0;" + "USE " + thisSelf.config.target_db.database + ";" + query + " SET sql_notes = 1;");
        });
        Promise.all(queries)
          .then(function() {
            resolve();
  ...
  migrationDatabaseProcess: function() {
    ...
    return new Promise(function(resolve, reject) {
      ...
      if (dumpExists && mysqlExists) {
        ...
        if ((thisSelf.config.source_db.user && thisSelf.config.source_db.password) || (thisSelf.config.source_db.user)) {
          dump =
            execDump +
